...
@CapabilityDescription("Retrieves a listing of files from HDFS. For each file that is listed in HDFS, creates a FlowFile that represents "
        + "the HDFS file so that it can be fetched in conjunction with ListUDS. This Processor is designed to run on Primary Node only "
...
@SeeAlso({GetUDS.class, FetchUDS.class, PutUDS.class})
public class ListUDS extends AbstractHadoopProcessor {
